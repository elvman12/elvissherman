...
function ppbBuildItem()
{
	...
	jQuery("#content_builder_sort li a.ppb_edit").click(function(){
		...
		jQuery.each(currentItemOBJ, function(index, value) { 
			if(typeof jQuery('#'+index).attr('multiple') != 'undefined')
			{
				var selectOptions = jQuery.parseJSON(decodeURI(value));
				
				jQuery.each( selectOptions, function( key, value ) {
					jQuery('#'+index+' option[value="'+value+'"]').prop('selected', true);
				});
			}
			else if(typeof jQuery('#'+index) != 'undefined')
			{
...
jQuery(document).ready(function(){
    ...
    if(self.document.location.hash != '')
	{
	    ...
	    jQuery('.rm_opts div'+self.document.location.hash).css('display', 'block');
    ...
    jQuery(".pp_font").each(function(){
		...
		jQuery("#ppb_sortable_add_button").click(function(){
			...
			if(myCheckId != '')
			{
				...
				builderItem+= '<input type="hidden" class="ppb_setting_columns" value="one"/>';
				builderItem+= '</li>';
			 	...
			 	if(typeof jQuery(this).attr('id') != 'undefined')
			 	{
			 		...
			 		{			 		
			 			...
			 			jQuery(this).find('option:selected').each(function(i, selected){ 
							selectedValues[i] = jQuery(selected).val();
						});
						
						itemData[jQuery(this).attr('id')] = encodeURI(JSON.stringify(selectedValues));
			 		}
			 		else
			 		{
